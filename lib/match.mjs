 ...
 * @property {string[]} [siblingRoots]  Other worktree roots of the same repo — in-lane
...
export function match(prompt, pin, knownProjects = []) {
  ...
  // An alias claimed by two or more DIFFERENT projects names a convention, not
  // a lane. Auto-generated alias lists produce these in bulk (claude-config-sync,
  // claude-code-lane-lock and claude-n8n-expert all claim "claude"), and one
  // such token blocks nearly every prompt while identifying no project in
  // particular. Ambiguous aliases never block.
  ...
  for (const project of knownProjects) {
    for (const alias of project.aliases || []) {
      ...
      aliasOwners.get(key).add(project.name);
    }
  }
  ...
  for (const project of knownProjects) {
    ...
    for (const alias of project.aliases) {
      ...
      if ((aliasOwners.get(alias.toLowerCase())?.size ?? 0) > 1) continue; // claimed by 2+ projects
  ...
  // Rule 3: absolute path extraction. Any path mentioned in the prompt that is
  // NOT inside the pinned root — or one of its sibling worktrees, which are the
  // same repository — is drift evidence.
  const inLaneRoots = [pin.root, ...(pin.siblingRoots || [])].filter(
  ...
  for (const m of prompt.matchAll(ABS_PATH_REGEX)) {
    ...
    try {
      if (!inLaneRoots.some((root) => isInside(candidate, root))) {
