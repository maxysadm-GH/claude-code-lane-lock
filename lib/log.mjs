...
// LANE_LOCK_LOG_DIR lets tests (and sandboxes) redirect the append-only log.
// Without it the e2e suite wrote fixture projects into the user's real
// drift.log.jsonl, which then polluted downstream rollups. Fixed 2026-08-21.
const LOG_DIR = process.env.LANE_LOCK_LOG_DIR || join(homedir(), '.claude', 'lane-lock');
