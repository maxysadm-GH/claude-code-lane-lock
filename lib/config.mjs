 ...
 * 3. Sibling git worktrees of the SAME repository are folded in as aliases.
 *    A worktree is one project checked out twice, so a knownProjects entry
 *    registered at the main checkout describes this project, not a neighbour.
 *
 ...
 * @param {string[]} [siblingRoots] Other worktree roots of the same repository
 * @returns {{ name: string, aliases: string[] }}
 */
export function inferPinNameFromRoot(pinRoot, config, siblingRoots = []) {
  const selfRoots = [pinRoot, ...(siblingRoots || [])].filter(
    (r) => typeof r === 'string' && r.length > 0
  );

  // Prefer an explicit config match on the pin root itself for the canonical name.
  const matched = (config.knownProjects || []).find(
    (p) => p.root && normalizeForCompare(p.root) === normalizeForCompare(pinRoot)
  );
  const name = matched ? matched.name : basename(pinRoot).toLowerCase();

  // Every knownProjects entry registered at the pin root OR at one of its
  // sibling worktrees describes this same repository — it is us, not a
  // neighbour. Folding those aliases into the pin is what stops a session
  // pinned to an Orca worktree (basename 'HERMES') from blocking every prompt
  // that names its own repo.
  const selfProjects = (config.knownProjects || []).filter(
    (p) =>
      p.root && selfRoots.some((r) => normalizeForCompare(p.root) === normalizeForCompare(r))
  );

  const aliases = Array.from(
    new Set([
      name,
      ...selfProjects.flatMap((p) => [p.name, ...(p.aliases || [])]),
      ...selfRoots.map((r) => basename(r).toLowerCase()),
    ])
  ).filter((a) => typeof a === 'string' && a.length >= MIN_ALIAS_LENGTH);
